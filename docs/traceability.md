...
| `valid` ⇔ non-empty | `a_valid_iff_not_empty` | BMC 20 | — |
| Ring invariants / no over-underflow | `a_count_*`, `a_no_overflow/underflow` | BMC 20 | `c_reach_full`, `c_full_then_empty` |
...
| Requirement | Property / check | Method | Witness |
|-------------|------------------|--------|---------|
| Data crosses the width change in FIFO order, correct sub-word position | `a_width_data_integrity` (`$anyconst` narrow-beat tracker) | BMC 14 | `c_track_roundtrip` + `sim-width-fifo-fault` |
...
| Correct for up/down-size, ratios 2:1–8:1, both sub-word orders | narrow-granularity golden model sweep | SIM | `sim-width-fifo-fault` |
...
- **Anti-vacuity:** every scoreboard has a fault-injection self-test that must
  ...
  `sim-fwft-fault`, `sim-width-fifo-fault`.
